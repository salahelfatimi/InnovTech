      ...
      <div className='flex flex-col gap-8'>
        {laboratoire?.map((ele, index) => (
          <div className='  ' key={index}>
            <div> 
              <div className={`flex justify-between items-center bg-[#ff9825] p-4   drop-shadow-2xl ${openItem === ele.id || ele.etablissment===establishment ? 'rounded-t-3xl' : 'rounded-3xl'}`}>
                <span className='font-bold text-lg text-white'>{ele.etablissment}</span>
                <button onClick={() => toggleOpen(ele.id)} className={`transition duration-700 text-white ${openItem === ele.id || ele.etablissment===establishment ? 'rotate-0' : 'rotate-180'}`}>
                  <ChevronDown size={30} className=' stroke-2'/>  
                </button>
              </div>
              <div className={` relative rounded-b-3xl   bg-[#FFFFFF] border-b-2 border-x-2 border-[#ff9825] dark:bg-[#1c1f24] p-4 drop-shadow-2xl  ${openItem === ele.id || ele.etablissment===establishment ? 'block' : 'hidden'}`}>
                <div>
